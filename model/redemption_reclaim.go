...
func settleRedemptionQuotaRowsTx(tx *gorm.DB, userId int, due []Redemption, now int64) (int, int, error) {
	...
	total64 := int64(0)
	for i := range due {
		if due[i].Quota <= 0 || due[i].Quota > common.MaxWalletQuota ||
		...
		if int64(due[i].ReclaimRemainingQuota) > int64(common.MaxWalletQuota)-total64 {
...
func allocateTimedQuotaTx(tx *gorm.DB, userId int, amount int, now int64) (WalletQuotaAllocation, error) {
	...
	for i := range redemptions {
		if redemptions[i].Quota <= 0 || redemptions[i].Quota > common.MaxWalletQuota ||
...
func refundTimedQuotaTx(tx *gorm.DB, userId int, amount int, allocation *WalletQuotaAllocation, now int64) error {
	...
	for i := len(allocation.Segments) - 1; i >= 0 && remaining > 0; i-- {
		...
		if redemption.Quota <= 0 || redemption.Quota > common.MaxWalletQuota ||
...
func RefundUserQuotaWithTimedAllocation(userId int, amount int, allocation *WalletQuotaAllocation) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		currentAfterReclaim := int64(user.Quota) - int64(reclaimed)
		if currentAfterReclaim > int64(common.MaxWalletQuota)-int64(amount) {
...
func SumActiveLimitedQuotaTx(tx *gorm.DB, userId int, now int64, lock bool) (int, error) {
	...
	for i := range redemptions {
		...
		if int64(redemptions[i].ReclaimRemainingQuota) > int64(common.MaxWalletQuota)-total64 {
...
func GetLimitedQuotaSummary(userId int, now int64) (LimitedQuotaSummary, error) {
	...
	for i := range rows {
		if rows[i].Quota <= 0 || rows[i].Quota > common.MaxWalletQuota ||
		...
		groupedQuota := grouped[rows[i].ExpiredTime]
		if rows[i].ReclaimRemainingQuota > common.MaxWalletQuota-groupedQuota {
			return LimitedQuotaSummary{}, errors.New("limited quota group exceeds the supported quota range")
		}
		grouped[rows[i].ExpiredTime] = groupedQuota + rows[i].ReclaimRemainingQuota
	}
	...
	for _, expiredTime := range expirations {
		...
		if quota < 0 || quota > common.MaxWalletQuota-summary.Total {
